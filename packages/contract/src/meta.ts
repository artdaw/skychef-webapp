...
type PayoutData = { [accountId: string]: bigint };
type PayoutProps = { payout: PayoutData };

export class Payout {
  payout: PayoutData;
  constructor({ payout }: PayoutProps) {
    this.payout = payout;
  }
}

type NFTContractMetadataProps = {
  spec: string;
  name: string;
  symbol: string;
  icon?: string;
